...
namespace YAML {
  ...
  struct convert<DynamicLightSettings> {
    static Node encode(const DynamicLightSettings& rhs) {
      ...
      node["shadowmap-offset"] = rhs.dlsShadowMapOffset;
    ...
    static bool decode(const Node& node, DynamicLightSettings& rhs) {
      ...
      try {
        rhs.dlsShadowMapOffset = node["shadowmap-offset"].as<float>();
      } catch (...) {
        Logger::log(1, "%s warning: could not parse shadowmap offset, init with a default value\n", __FUNCTION__);
        rhs.dlsShadowMapOffset = defaultSettings.dlsShadowMapOffset;
      }
      if (rhs.dlsType == dynamicLightType::spot) {
        try {
