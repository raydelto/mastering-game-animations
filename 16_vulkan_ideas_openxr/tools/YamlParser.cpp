...
YAML::Emitter& operator<<(YAML::Emitter& out, const DynamicLightSettings& settings) {
  ...
  out << YAML::Key << "shadowmap-offset";
  out << YAML::Value << settings.dlsShadowMapOffset;
...
float YamlParser::getDynLightShadowMapConstantDepthBias() {
  float bias = 1.25f;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return bias;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "dynlight-shadowmap-constant-bias") {
        bias = it->second.as<float>();
      }
    }
  } catch (...) {
...
float YamlParser::getDynLightShadowMapSlopeDepthBias() {
  float bias = 1.75f;
  if (!hasKey("settings")) {
    Logger::log(1, "%s error: no settings found in config file '%s'\n", __FUNCTION__, mYamlFileName.c_str());
    return bias;
  ...
  try {
    for(auto it = settingsNode.begin(); it != settingsNode.end(); ++it) {
      if (it->first.as<std::string>() == "dynlight-shadowmap-slope-bias") {
        bias = it->second.as<float>();
      }
    }
  } catch (...) {
...
bool YamlParser::createConfigFile(VkRenderData renderData, ModelInstanceCamData modInstCamData) {
  ...
  mYamlEmit << YAML::Key << "dynlight-shadowmap-constant-bias";
  mYamlEmit << YAML::Value << renderData.rdDynLightShadowMapConstantDepthBias;
  mYamlEmit << YAML::Key << "dynlight-shadowmap-slope-bias";
  mYamlEmit << YAML::Value << renderData.rdDynLightShadowMapSlopeDepthBias;
  mYamlEmit << YAML::EndMap;
  mYamlEmit << YAML::EndMap;
