...
void main() {
  ...
  gl_Position = projectionMat[XR_VIEW_INDEX] * viewMat[XR_VIEW_INDEX] * worldPosSkinMat *
  ...
  vec3 origNormal = aNormal.xyz;
  vec3 morphNormal = morphVertices[gl_VertexIndex + morphAnimIndex].normal.xyz;
  normal = transpose(inverse(mat3(worldPosSkinMat))) *
