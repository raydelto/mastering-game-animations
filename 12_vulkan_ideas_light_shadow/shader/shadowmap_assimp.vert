...
layout (std430, set = 1, binding = 3) readonly restrict buffer ShadowMapCascadeParameters {
  ShadowMapCascadeData shadowMapData[];
};

void main() {
  mat4 modelMat = worldPosMat[gl_InstanceIndex + worldPosOffset];
  gl_Position = shadowMapData[shadowMapLayerIndex].shadowMapMat * modelMat * vec4(aPos.xyz, 1.0);
