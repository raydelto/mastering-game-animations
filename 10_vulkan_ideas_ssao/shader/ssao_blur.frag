...
layout (location = 6) out float outColor;
...
void main() {
  ...
  float depth = getPosFromDepth(inUV).z;
  ...
  for (int x = -ssaoBlurRadius; x <= ssaoBlurRadius; x++) {
    for (int y = -ssaoBlurRadius; y <= ssaoBlurRadius; y++)  {
      ...
      float offsetDepth = getPosFromDepth(inUV + offset).z;
      vec3 offsetNormal = (viewMat * vec4(normalize(texture(inputNormal, inUV + offset).rgb * 2.0 - 1.0), 0.0)).xyz;
