...
void InstanceMovementNode::calculateRotation() {
  ...
  if (mMaxRot < mMinRot) {
    ...
    if (std::fabs(mMaxRot - mMinRot) > 0.01f) {
      ...
      std::uniform_real_distribution<float> randomFloats(mMinRot * 100, (mMaxRot - mMinRot) * 100);

      mRotation = randomFloats(randomEngine);
...
void InstanceMovementNode::calculateSpeed() {
  ...
  if (mMaxSpeed < mMinSpeed) {
    ...
    if (std::fabs(mMaxSpeed - mMinSpeed) > 0.01f) {
      ...
      unsigned int seed = randomDevice();
      std::default_random_engine randomEngine(seed);
      std::uniform_real_distribution<float> randomFloats(mMinSpeed * 100, (mMaxSpeed - mMinSpeed) * 100);

      mSpeed = randomFloats(randomEngine);
